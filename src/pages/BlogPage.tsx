...
interface Props { categoryId: string; postId: string }
...
export default function BlogPage({ categoryId, postId }: Props) {
  ...
  if (!category || !post) { navigate(categoryId); return null }
      ...
      <header style={{ background: theme.surface, borderBottom: `1px solid ${theme.border}`, padding: '22px 0' }}>
        <div className="mx-auto max-w-3xl px-8 flex items-center justify-between gap-4">
          ...
          <button
            ...
            style={{ fontFamily: "'Inter', sans-serif", fontSize: '12px', color: theme.accent, background: theme.accentSoft, padding: '4px 10px', borderRadius: '2px', border: 'none', cursor: 'pointer' }}
          >
            {category.title}
          </button>
        </div>
      ...
      <article>
        <header className="relative overflow-hidden" style={{ padding: '72px 0 64px', background: `radial-gradient(ellipse at 85% 10%, ${theme.accentSoft}, transparent 55%)`, borderBottom: `1px solid ${theme.border}` }}>
          ...
          <div className="mx-auto max-w-3xl px-8">
            ...
            <h1 style={{ fontFamily: theme.headingFont, fontWeight: 600, fontSize: 'clamp(32px, 5vw, 52px)', lineHeight: 1.1, letterSpacing: '-0.02em', color: theme.text, margin: 0, maxWidth: '24ch' }}>
              {post.title}
            </h1>
          </div>
        </header>
      ...
      {(prevPost || nextPost) && (
        <nav style={{ borderTop: `1px solid ${theme.border}`, padding: '40px 0' }}>
          <div className="mx-auto max-w-3xl px-8" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '32px' }}>
            {prevPost ? (
              <button onClick={() => navigate(categoryId, prevPost.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, textAlign: 'left' }}>
                <p style={{ fontFamily: "'Inter', sans-serif", fontSize: '12px', color: theme.textMuted, marginBottom: '8px' }}>← Previous</p>
                <p style={{ fontFamily: theme.headingFont, fontSize: '16px', color: theme.textSoft, margin: 0 }}>{prevPost.title}</p>
              </button>
            ) : <div />}
            {nextPost ? (
              <button onClick={() => navigate(categoryId, nextPost.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, textAlign: 'right' }}>
                <p style={{ fontFamily: "'Inter', sans-serif", fontSize: '12px', color: theme.textMuted, marginBottom: '8px' }}>Next →</p>
                <p style={{ fontFamily: theme.headingFont, fontSize: '16px', color: theme.textSoft, margin: 0 }}>{nextPost.title}</p>
              </button>
            ) : <div />}
          </div>
        </nav>
